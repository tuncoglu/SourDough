...
export function LactoAdvice({ advice, tips, presetEmoji, presetName }: Props) {
  ...
  return (
    <View style={[cardStyleLg, { backgroundColor: colors.card, borderColor: colors.border }]}>
      ...
      {/* Safety advice */}
      {advice.map((line, i) => (
        <Text key={`a-${i}`} style={[styles.line, { color: line.startsWith('⚠️') ? colors.hot : line.startsWith('🧂') ? colors.warm : colors.muted }]}>
          {line}
        </Text>
      ))}
